...
extension CafesViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let celulaDeCafe = celula as? CafeTableViewCell {
            let item = capsulas[indexPath.section].cafes[indexPath.row]
            let presenter = CafeTableViewCellPresenter(cafe: item, tela: celulaDeCafe)
            celulaDeCafe.presenter = presenter
            celulaDeCafe.configurar()
        }
